...
void computeKthDeri(Node var, Node def)
{
  ...
  switch(code)
      ...
      if(NODE_IS_CST(right))
	{
	  ...
	  if(left == timeVar)
	    {
	      if(icnt1&1)fprintf(outfile, "\t\t if(_jz_k==1) {DivideMyFloatA(%s, MakeMyFloatB(_jz_tvar1, 1), %s);}\n", name1, rightStr);
	      else  fprintf(outfile, "\t\t if(_jz_k==1) {DivideMyFloatA(%s, MakeMyFloatB(_jz_uvar1, 1), %s);}\n", name1, rightStr);
	      icnt1++;
	      fprintf(outfile, "\t\t else {AssignMyFloat(%s, _jz_MyFloatZERO);}\n", name1);	      
	    }
	...
	if(!strcmp(fname, "sin"))
	...
	else if(!strcmp(fname, "sqrt"))
	  {
	    ...
	    fprintf(outfile, "\t\t\t     MultiplyMyFloatA(tmp1, %s,%s);\n", str, rstr);
	    fprintf(outfile, "\t\t\t     SubstractMyFloatA(tmp3, tmp, tmp1);\n");
	    fprintf(outfile, "\t\t\t     AssignMyFloat(tmp, tmp3);\n");
	    fprintf(outfile, "\t\t\t }\n");
