...
{\n\
  ...
  double compute_stepsize_1__ODE_NAME__(MY_FLOAT**, int, double, int);\n\
  double compute_stepsize_2__ODE_NAME__(MY_FLOAT**, int, double, int);\n\
  double comp_stepsize_other__ODE_NAME__(MY_FLOAT**, int, int, double, double, double);\n\
\n\
...
*/\n\
  if(step_ctl != 0) {\n\
    s=taylor_coefficients__ODE_NAME__A(*ti,x,nt,1);\n\
  } else {\n\
    s=taylor_coefficients__ODE_NAME__(*ti,x,nt); \n\
 }\n\
\n\
/*\n\
...
*/\n\
  switch(step_ctl)\n\
    ...
    case 0: /* no step size control (fixed step size, given by the user) */\n\
      ...
      break;\n\
    case 1:\n\
      dh=compute_stepsize_1__ODE_NAME__(s,nt,xnorm,flag_err);\n\
      MakeMyFloatA(h, dh);\n\
      break;\n\
    case 2:\n\
      dh=compute_stepsize_2__ODE_NAME__(s,nt,xnorm,flag_err);\n\
      MakeMyFloatA(h, dh);\n\
      break;\n\
    case 3:\n\
      dh=comp_stepsize_other__ODE_NAME__(s,_N_DIM_,nt,xnorm,log10abserr,log10relerr);\n\
      MakeMyFloatA(h, dh);\n\
      break;\n\
    default:\n\
      fprintf(stderr, \"taylor_step: undefined step size control.\\n\");\n\
...
double compute_stepsize_1__ODE_NAME__(MY_FLOAT **s, int nt, double xnorm, int flag_err)\n\
/*\n\
...
double compute_stepsize_2__ODE_NAME__(MY_FLOAT **s, int nt, double xnorm, int flag_err)\n\
/*\n\
 * it looks for a step size for an expansion up to order nt. this\n\
 * function requires that nt is the value computed by\n\
